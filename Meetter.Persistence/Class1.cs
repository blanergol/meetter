...
public sealed class FileMeetingsCache : IMeetingsCache
{
	...
	{
		_cacheDir = cacheDir ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Meetter", "cache");
	}

	public async Task<(bool isHit, IReadOnlyList<Meeting> meetings)> TryReadAsync(DateTimeOffset from, DateTimeOffset to)
	{
