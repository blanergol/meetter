...
{
	private static readonly Regex GoogleRedirect = new(@"https?:\/\/www\.google\.com\/url\?q=([^&\s]+)", RegexOptions.IgnoreCase | RegexOptions.Compiled);
...
{
	private static readonly Regex MeetRegex = new(@"https?:\/\/meet\.google\.com\/(?:lookup\/)?[a-z0-9\-]+(?:\?[\w\-=&%]+)?(?:#[\w\-=&%]+)?", RegexOptions.IgnoreCase | RegexOptions.Compiled);
	...
	public bool TryExtractJoinUrl(string text, out string? url)
	{
		...
		if (string.IsNullOrWhiteSpace(text)) { url = null; return false; }
		var match = MeetRegex.Match(text);
		if (match.Success)
		{
			url = match.Value.StartsWith("http", StringComparison.OrdinalIgnoreCase) ? match.Value : "https://" + match.Value;
			return true;
		}
		url = null;
		return false;
...
{
	private static readonly Regex ZoomRegex = new(@"https?:\/\/(?:[\w\.-]+\.)?zoom\.us\/(?:j|my|wc\/join)\/[^\s]+", RegexOptions.IgnoreCase | RegexOptions.Compiled);
